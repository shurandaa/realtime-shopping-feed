...
export const api = {
  ...
  getRecommendationList: async (page: number = 1, pageSize: number = 12): Promise<{ products: Product[]; hasMore: boolean; total: number }> => {
    ...
    const allProducts = response.data.data.products.map((p: any) => ({
      id: String(p.id),
      name: p.name,
      category: p.category,
      price: p.price,
  ...
  getDetail: async (productId: string): Promise<Product & { recommendedList: Product[] }> => {
    ...
    const product: Product = {
      ...
      price: data.price,
      description: data.description || `High-quality ${data.name.toLowerCase()}`,
    ...
    if (data.relatedImages && data.relatedImages.length > 0) {
      ...
      recommendedList = data.relatedImages.slice(0, 4).map((img: string, idx: number) => ({
        id: `related_${idx}`,
      ...
      recommendedList = allProducts
        .filter((p: any) => String(p.id) !== productId && p.category === data.category)
        .slice(0, 4)
        .map((p: any) => ({
          id: String(p.id),
          name: p.name,
